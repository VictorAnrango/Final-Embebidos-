...
static float cached_spo2_max = 100.0f;
    ...
    if (err == ESP_OK && thresholds_buf_len > 0)
    {
        ...
        if (root)
        {
            if (xSemaphoreTake(thresholds_mutex, pdMS_TO_TICKS(200)) == pdTRUE)
            {
                ...
                if (item) cached_spo2_max = (float)item->valuedouble;
            ...
            ESP_LOGI(TAG, "Umbrales: bpm_max=%d spo2_max=%.1f mov_max=%.2f",
                     cached_bpm_max, cached_spo2_max, cached_mov_max);
